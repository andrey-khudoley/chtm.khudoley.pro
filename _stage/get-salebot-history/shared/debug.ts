...
export class Debug extends Error {
  ...
  /** Лог уровня `info`. */
  static info(ctx: app.Ctx, message: string): void {
    // eslint-disable-next-line no-new
    new Debug(ctx, message, 'info');
  }

  /** Лог уровня `warn`. */
  static warn(ctx: app.Ctx, message: string): void {
    // eslint-disable-next-line no-new
    new Debug(ctx, message, 'warn');
  }

  /** Лог уровня `error` без выброса. */
  static error(ctx: app.Ctx, message: string, code?: string): Debug {
    return new Debug(ctx, message, 'error', code);
  }

  /** Выброс ошибки. */
  static throw(ctx: app.Ctx, message: string, code?: string): never {
  ...
  constructor(ctx: app.Ctx, message: string, level: DebugLevel = 'error', code?: string) {
    super(message);
    this.name = 'Debug';
    this.code = code;
  ...
  private write(ctx: app.Ctx, message: string, level: DebugLevel, code?: string): void {
    const ts = new Date().toISOString();
    const head = `[${level.toUpperCase()}][${ts}]${code ? `[${code}]` : ''}`;
    const line = `${head}: ${message}`;

    try { 
      ctx.account?.log?.(line, { level: 'info' }); 
    } catch {}
  }
}
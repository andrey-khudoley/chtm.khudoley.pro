...
import { ChatiumAction, ChatiumActions, SubmitFormAction } from 'lib/chatium-json'
import { Icon } from 'lib/chatium-json/v1'
import { DesktopLayout, PlainIconName, SmartIconProps } from 'lib/chatium-json/v2'
import {
...
export declare const systemOnlyIdentityTypes: readonly ['Password', 'ApiKey', 'DeviceId', 'TechSupport', 'GetcourseId']
...
interface SubmitEmailAndSendCodeActionParams {
  back?: string
  fromApiCall?: boolean
  codeSentScreenUrl?: string
  openCodeSentScreenInModal?: boolean
  formId?: SubmitFormAction['formId']
}
interface SubmitEmailCodeActionParams {
  email: string
  back?: string
  fromApiCall?: boolean
  afterAuthAction?: ChatiumActions
  verificationId?: string
  fromModal?: boolean
}
export declare function getEnabledAuthProviders(ctx: RichUgcCtx): Promise<Record<AuthProvider, AnyObject>>
export declare function getAuthProvidersSettings(
  _ctx: RichUgcCtx,
): Promise<ReturnType<UgcAuthServiceApi['getAuthProvidersSettings']>>
export declare function updateAuthProviderSettings(
  _ctx: RichUgcCtx,
  provider: AuthProvider,
  patch: Parameters<UgcAuthServiceApi['updateAuthProviderSettings']>[1],
): Promise<ReturnType<UgcAuthServiceApi['updateAuthProviderSettings']>>
export declare function deleteAuthProviderSettings(_ctx: RichUgcCtx, provider: AuthProvider): Promise<void>
export declare function getAvailableCustomAuthProviders(ctx: RichUgcCtx): Promise<AuthProviderInfo[]>
declare type CreateAuthSessionByConfirmedIdentityResult =
  | CreateAuthSessionResultCreated
  | CreateAuthSessionResultRedirect
export declare function createOrUpdateAuthSessionByConfirmedIdentityInCurrentSession(
  ctx: RichUgcCtx,
  identityType: CustomProvidableIdentityType,
  identityKey: string,
  provider: AuthProvider | AuthProviderProps,
  cookieOptions?: CookieOptions,
): Promise<CreateAuthSessionByConfirmedIdentityResult>
export declare function createOrUpdateAuthSessionByConfirmedIdentityInGivenSession(
  ctx: RichUgcCtx,
  identityType: CustomProvidableIdentityType,
  identityKey: string,
  provider: AuthProvider | AuthProviderProps,
  deviceId: string,
  targetAuthSessionId?: string,
): Promise<string>
/**
 * Allows auth-provider logic to preliminarly create a confirmed user-identity pair without an auth session.
 * Returns the existing user if the identity already exists, or creates a new user with the confirmed identity.
 */
export declare function createUserWithConfirmedIdentity(
  ctx: RichUgcCtx,
  identityType: CustomProvidableIdentityType,
  identityKey: string,
  provider: AuthProvider | AuthProviderProps,
): Promise<CreateUserWithIdentityResult>
export declare function createConfirmedIdentity(
  _ctx: RichUgcCtx,
  userOrId: UgcSmartUser | string,
  identityType: CustomProvidableIdentityType,
  identityKey: string,
  provider: AuthProvider | AuthProviderProps,
): Promise<UgcIdentity>
/**
 * Sets cookie with auth session which has just been created via external provider (browser) for this device.
 */
export declare function acquireAuthSessionForDevice(ctx: RichUgcCtx, authSessionId: string): Promise<void>
/**
 * Returns actions that should be performed on the client when
 *  createOrUpdateAuthSessionByConfirmedIdentity is successfully called and the auth cookie is set.
 */
export declare function finalizeLoginActions(
  ctx: RichUgcCtx,
  backUrl?: string,
  fromApiCall?: boolean,
  fromModal?: boolean,
...
/**
 * Returns smartly prepared submitForm action to correctly send SMS confirmation code to the entered phone,
 *  depending on the given params, account settings and client app.
 * Phone input name in the form must be 'phone'.
 * This function is helpful for creating custom sign-in screens.
 */
export declare function submitPhoneAndSendSmsCodeAction(
  ctx: RichUgcCtx,
  {
    back,
    fromApiCall,
    codeSentScreenUrl,
    openCodeSentScreenInModal,
    formId,
  }: {
    back?: string
    fromApiCall?: boolean
...
/**
 * Returns smartly prepared submitForm action to correctly submit SMS code entered by the user and complete phone
 *  authentication, depending on the given params, account settings and client app.
 * SMS code input name in the form must be 'validationCode'.
 * This function is helpful for creating custom sign-in screens.
 */
export declare function submitSmsCodeAction(
  ctx: RichUgcCtx,
  {
    phone,
    back,
...
/**
 * Returns smartly prepared submitForm action to correctly send SMS confirmation code to the entered email,
 *  depending on the given params, account settings and client app.
 * Email input name in the form must be 'email'.
 * This function is helpful for creating custom sign-in screens.
 */
export declare function submitEmailAndSendCodeAction(
  ctx: RichUgcCtx,
  params: SubmitEmailAndSendCodeActionParams,
): Promise<SubmitFormAction>
/**
 * Returns smartly prepared submitForm action to correctly submit code entered by the user and complete email
 *  authentication, depending on the given params, account settings and client app.
 * SMS code input name in the form must be 'code'.
 * This function is helpful for creating custom sign-in screens.
 */
export declare function submitEmailCodeAction(
  ctx: RichUgcCtx,
  params: SubmitEmailCodeActionParams,
): Promise<SubmitFormAction>
export type { UgcAuthProviderSettings as AuthProviderSettings } from 'services/ugc-runner/runtime/runtimeTypes'
